...
void EditorController::addEditor(QString path){
    ...
    if(!dir.exists()){
        ...
        setSyntaxController(type,e,path);
    }
}
...
void EditorController::setSyntaxController(QString &type, Editor *e, QString path){
    if(!type.compare("xml")){
        XMLController *controller = new XMLController(e, path);
